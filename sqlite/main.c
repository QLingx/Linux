...
#define IS_ROOT_OFFSET NODE_TYPE_OFFSET
...
typedef enum MetaCommandResult_t{
    ...
    META_COMMAND_UNRECOGNIZED_COMMADN
}MetaCommandResult;
...
void internal_node_insert(Table *table,uint32_t parent_page_num,uint32_t child_page_num){
    ...
    uint32_t origin_num_keys = *internal_node_num_keys(parent);
    *internal_node_num_keys(parent) = origin_num_keys + 1;
    ...
    if(child_max_key > get_node_max_key(right_child)){
        *internal_node_child(parent,origin_num_keys) = right_child_page_num;
        *internal_node_key(parent,origin_num_keys) = get_node_max_key(right_child);
        *internal_node_right_child(parent) = child_page_num;
    }else{
        for(uint32_t i = origin_num_keys;i > index ;i--){
...
MetaCommandResult do_meta_command(InputBuffer *input_buffer,Table *table){
    if(strcmp(input_buffer->buffer,".exit") == 0){
        ...
        return META_COMMAND_UNRECOGNIZED_COMMADN;
    }
}
        ...
        if(input_buffer->buffer[0] == '.'){
            switch(do_meta_command(input_buffer,table)){
                case META_COMMAND_SUCCESS:
                    continue;
                case META_COMMAND_UNRECOGNIZED_COMMADN:
